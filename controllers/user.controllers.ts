...
export const registrationUser = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { name, email, password, number, captcha } = req.body;
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (newUser.activationCode !== activation_code) {
        return next(new ErrorHandler("Inavalid Activation code", 400));
      }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (email && user) {
        ...
        if (isEmailExist) {
          console.log(`${next(new ErrorHandler("email already exist", 400))}`);
...
export const updatePassword = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.log(req.body as IUpadtePassword);
